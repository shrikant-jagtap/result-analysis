...
self.location='cleanpass.php?cat=' + val ;
}
...
self.location='cleanpass.php?cat=' + val + '&cat3=' + val2 ;
}
function vis(){
...
<h1 style="text-align:center;color:#972021;font-weight:normal;">Clean Pass</h1>
<?php
if(!isset($_POST['show_clean']))
{
?>
<form action="cleanpass.php" method="post">
...
<input type="submit" id="ro" name="show_clean" value="Show">
...
if(isset($_POST['show_clean']))
{
$query="select * FROM student WHERE roll_no IN (SELECT DISTINCT roll_no FROM student_exam WHERE roll_no IN (SELECT DISTINCT roll_no FROM student_exam WHERE sem_no<=".$_POST['engg_year']."*2 AND result LIKE 'P%') AND exam_year='".$_POST['year']."' AND engg_year=".$_POST['engg_year'].")";
$result=mysql_query($query);
?>
<table id="Clean Pass" cellspacing="30">
  <tr>
   <th>Name</th>
   <th>Roll No</th>
    </tr>
...
<?php 
   ...
   ?>
   <button onclick="print_result()">Print</button>
   <?php
}
?>
</div>
