...
<h1 style="text-align:center">Passed Students</h1>
<?php
if(!isset($_POST['pass']))
{
?>
<form action="passstud.php" method="post">
...
<input id="ro" type="submit" name="pass" value="Show">
...
if(isset($_POST['pass']))
{
$query_pass="SELECT * FROM student where roll_no IN (SELECT DISTINCT roll_no FROM student_exam WHERE result='P' or result='P*' AND exam_year='".$_POST['year']."' AND engg_year=".$_POST['engg_year']." AND sem_no=".$_POST['engg_year']."*2-1)";
$result_pass=mysql_query($query_pass);
?>
<table id="Pass" cellspacing="30">
  <tr>
   <th>Name</th>
   <th>Roll No</th>
    </tr>
	<?php
	while ($row = mysql_fetch_array($result_pass))
{
 ?>
   <tr>
...
$ans=mysql_num_rows($result_pass);
echo "For ODD Sem : ".$ans;
$query_pass="SELECT * FROM student where roll_no IN (SELECT DISTINCT roll_no FROM student_exam WHERE result='P' AND exam_year='".$_POST['year']."' AND engg_year=".$_POST['engg_year']." AND sem_no=".$_POST['engg_year']."*2)";
$result_pass=mysql_query($query_pass);
?>
<table id="Pass" cellspacing="30">
  <tr>
...
?>
<button id="ro" onclick="print_result()">Print</button>
